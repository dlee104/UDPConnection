...
int32_t udp_server(int port)
{
    ...
    local.sin_port = htons(port); // tries to get port number 
    
    //bind the name (address) to a port 
    if (bindMod(sk, (struct sockaddr *)&local, sizeof(local)) < 0)
    {
        perror("in udp_server, bindMod");
        exit(-1);
    }
...
int32_t udp_client_setup(char *hostname, uint16_t port_num, Connection *connection)
{
    ...
    struct hostent *hp = NULL; //address of remote hose 
    connection->sk_num = 0; 
    ...
    {
        perror("in udp_client_setup, socket");
        exit(-1);
    }
